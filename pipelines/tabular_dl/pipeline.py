...
class TabularDLPipeline(BasePipeline):
    ...
    def _train_tabpfn(self, X: Any, y: Any, params: dict[str, Any]) -> Any:
        ...
        clf = TabPFNClassifier(device=self._device(), **{k: v for k, v in params.items()
                                                          if k in {"N_ensemble_configurations"}})
        clf.fit(X, y, overwrite_warning=True)
        return clf

    def _train_transformer(self, X: Any, y: Any, model_name: str,
                           params: dict[str, Any]) -> Any:
        try:
            import torch  # noqa: WPS433
            ...
            from pytorch_tabular.models import (
                FTTransformerConfig, TabTransformerConfig,
            )
        ...
        trainer_cfg = TrainerConfig(max_epochs=params.get("epochs", 20),
                                     batch_size=params.get("batch_size", 256),
                                     accelerator="auto")
        opt_cfg = OptimizerConfig(optimizer="Adam",
                                   optimizer_params={"lr": params.get("lr", 1e-3)})
        model_cfg = (TabTransformerConfig() if model_name == "TabTransformer"
                     else FTTransformerConfig())
        tm = TabularModel(data_config=data_cfg, model_config=model_cfg,
                          optimizer_config=opt_cfg, trainer_config=trainer_cfg)
        tm.fit(train=df_tr)
        return tm
