    ...
    async def _propose(self, state: PipelineState) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if arr:
                ...
                for p in arr:
                    p["outputs"] = [o for o in (p.get("outputs") or [])
                                     if o in ALL_OUTPUTS]
        ...
        defaults = [
            {"id": 1, "title": "발표 패키지",
             ...
             "rationale": "임원/실무 발표용 PPT+대본+대시보드", "score": 0.8},
            {"id": 2, "title": "리포트 패키지",
             "outputs": ["OUT-02", "OUT-07"],
             "rationale": "상세 PDF + 인사이트 요약", "score": 0.7},
            {"id": 3, "title": "최소 패키지",
             "outputs": ["OUT-04", "OUT-07"],
             "rationale": "온라인 공유용 대시보드+요약", "score": 0.6},
        ]
        if pre:
            defaults.insert(0, {"id": 0, "title": "사용자 사전 선택",
                                 "outputs": pre, "rationale": "사용자 지정",
                                 "score": 1.0})
        return defaults
