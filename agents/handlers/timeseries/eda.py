"""agents.handlers.timeseries.eda — 시계열 EDA 차트 (CS 담당, cs-day3 v2).

4 chart (+ missing 막대) MinIO 저장 + eda_summary 갱신:
  ① missing 막대 (Chart 0)
  ② line plot (Chart 1)
  ③ ACF + seasonal_period 추정 (Chart 2, statsmodels lazy import)
  ④ STL 4단 분해 (Chart 3, n ≥ 2s 가드)
  ⑤ seasonality heatmap year×month (Chart 4, DatetimeIndex 가드)

진입함수 (dispatcher 자동 등록):
  - charts(df, state) -> list[str]   MinIO 경로 list

DoD: 4 chart MinIO 저장 + eda_summary 갱신 (seasonal_period · acf_peaks · stationary).

핵심 설계 원칙:
  - 인라인 try/except 우선 — 각 차트 독립
  - 자연 발생 롤백 3 개만 (target/n_rows/모든 실패) — A-1/A-2/G-1 가드
...
def charts(df: Any, state: Any) -> list[str]:
    ...
    try:
        charts.last_eda_summary = {  # type: ignore[attr-defined]
            "charts": list(paths),
            "seasonal_period": s,
            "acf_peaks": acf_peaks,
            **eda_extras,
