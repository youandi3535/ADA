"""agents.handlers.timeseries.eda — 시계열 EDA 차트 (A 담당).

기본: 시계열 plot + 결측 막대.
Day 1 (A): ACF/PACF/STL/seasonality heatmap 추가 예정.
"""
...
def charts(df: Any, state: Any) -> list[str]:
    ...
    try:
        ...
        if not miss.empty:
            ...
            paths.append(save_chart_to_minio(fig, kind="timeseries/missing",
                                              job_id=state.job_id))
    except Exception:
        pass
    ...
    if target and target in df.columns:
        try:
            ...
            paths.append(save_chart_to_minio(fig, kind="timeseries/line",
                                              job_id=state.job_id))
        except Exception:
            pass
