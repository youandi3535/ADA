    ...
    async def __call__(self, state: PipelineState) -> PipelineState:
        async with self.log_agent_run(state):
            ...
            try:
                ...
                return state.with_update(error=f"학습 데이터 로딩 실패: {e}",
            ...
            if state.category == "timeseries":
                ...
                X_tr, X_val, y_tr, y_val = train_test_split(
                    ...
                    stratify=y if state.category in ("tabular_ml", "tabular_dl")
                              and len(set(y.tolist())) <= 20 else None,
                )

            pipeline = PipelineFactory.create(state.category)
            task = ("classification" if state.category in ("tabular_ml", "tabular_dl")
                    and len(set(y.tolist())) <= 20 else "regression")
            if state.category == "timeseries":
                task = "forecasting"
            ...
            for model_name in state.model_candidates:
                try:
                    ...
                    info = {"model_name": model_name, "metrics": metrics,
            ...
            return state.with_update(trained_models=trained,
