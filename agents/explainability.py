...
class ExplainabilityAgent(BaseAgent):
    ...
    async def _shap(self, state: PipelineState) -> dict[str, Any]:
        ...
        try:
            ...
            with open(tmp.name, "wb") as f:
                # MinIO 에서 다운로드
                f.write(mc.download_bytes(best["minio_path"]
                                          .replace(f"s3://{mc.bucket}/", "")))
        ...
        try:
            ...
            sv = explainer(X[:200])
            top_features = (np.abs(sv.values).mean(axis=0)
                            if sv.values.ndim == 2
                            else np.abs(sv.values).mean(axis=(0, 2)))
            order = np.argsort(-top_features)
            ranking = [{"feature": str(X.columns[i]),
                        "importance": float(top_features[i])}
                       for i in order[:20]]
    ...
    async def _timeseries_decompose(self, state: PipelineState) -> dict[str, Any]:
        ...
        try:
            ...
            return {"timeseries_decompose_path": path,
                    "period": period}
        except Exception as e:
