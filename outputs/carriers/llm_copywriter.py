...
12-4. 적용·로드맵 특칙 (id "i3_roi", "roadmap") — 이 장들은 ML 기술 운영
   (재학습 트리거·drift·앙상블)이 아니라 *분석 결과가 현실에서 어떤 행동·정책으로
   이어지는가*를 쓴다. user_intent·도메인을 보고 구체화하라.
   나쁨: "drift 0.1 초과 시 재학습 트리거" / "CatBoost 앙상블 적용"
   좋음: "선실 등급별 안전 대응을 차등화" / "여성·아동 우선 구조 원칙을 매뉴얼에 명문화"
   측정 가능한 게이트(성공 기준)는 유지하되, 행동의 주어는 *현업·정책*이어야 한다.
