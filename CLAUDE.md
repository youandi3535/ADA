...
## 🔁 10. 수정 후 점검·배포 표준 절차 (매 수정 필수)

> 사용자 고정 요청: 모든 수정 작업이 끝나면 아래 절차로 점검·조치하고,
> 사용자가 웹 대시보드에서 **새로고침(F5) 한 번**이면 모든 수정이 반영되게 한다.

### 10-1. F5 한 번 반영 보장 구조 (설정 완료)
- `docker/docker-compose.yml` frontend: `../frontend:/app/frontend` 라이브 마운트 → 호스트 수정 즉시 컨테이너 반영 (이미지 재빌드 불필요).
- `.streamlit/config.toml`: `fileWatcherType="poll"` + `runOnSave=true` → Windows 바인드 마운트에서도 변경을 폴링으로 감지 → 재시작 없이 새 코드 로드.
- `../.streamlit:/app/.streamlit:ro` 마운트 → 위 설정도 컨테이너에 라이브 반영.
- ⚠️ **최초 1회만** 컨테이너 재생성 필요: `cd docker && docker compose --profile core up -d frontend`. 이후 `frontend/app.py` 수정은 전부 F5(또는 자동 rerun)만으로 반영됨.

### 10-2. 매 수정 후 (Claude 가 직접 수행)
1. `python -m py_compile <수정파일>` → 문법 OK 확인
2. 수정 내용 grep/Read 로 실제 반영 확인
3. 결과를 10-4 표 양식으로 한글 보고

### 10-3. 컨테이너 전수 점검 (호스트에서 실행)
`bash scripts/dev/verify_frontend.sh ["확인할_변경문자열"]`
→ 호스트/컨테이너 py_compile, 호스트↔컨테이너 SHA256, grep 반영, docker ps 중복, health 200 을 자동 점검·표 출력.
(Claude 샌드박스에는 docker 가 없어 컨테이너 점검은 이 스크립트가 호스트에서 대신 수행한다.)

### 10-4. 보고 양식 (고정)

| # | 점검 항목 | 방법 | 결과 |
|---|---|---|---|
| 1 | 코드 문법 | py_compile (호스트+컨테이너) | OK |
| 2 | 마운트 동기화 | 호스트↔컨테이너 SHA256 | 일치 |
| 3 | 변경 반영 | 컨테이너 grep | 검출 |
| 4 | 중복 컨테이너 | docker ps -a (8501) | ada-frontend 1개 |
| 5 | 서비스 응답 | /_stcore/health | HTTP 200 |

마지막 줄에 "웹 대시보드 새로고침(F5) 한 번이면 반영됨" 을 명시한다.

### 10-5. 답변 언어 (고정)
- **모든 답변과 분석(사고) 과정은 한글로 작성**한다. 코드·파일명·플래그 등 식별자는 원문을 유지하되, 설명·사고 문장은 예외 없이 한글로 쓴다.
